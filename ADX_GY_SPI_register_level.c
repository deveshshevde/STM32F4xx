...
{
	SPI1->DR = byte;
	while ((SPI1->SR) & (1 << 1));	/* Wait till TX Buffer is empty */
	while ((SPI1->SR) & (1 << 7));	//Wait till SPI is busy
	buf = SPI1->DR;
	return buf;
}
void Accel_Write(uint8_t address,uint8_t val)
{
	GPIOA->ODR &= ~(1UL << 4);//Select accelerometer
	SPI_Send(address);
	SPI_Send(val);
	GPIOA->ODR |= (1UL << 4);//Disconnect accelerometer
}
